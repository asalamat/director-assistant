...
- **🖼 Image model selector** — Settings → LinkedIn now has an "Image Generation Model" dropdown (DALL-E 3 default, GPT Image 1, GPT-5.5, DALL-E 2); the app tries your preferred model first and auto-falls back through the list if the model isn't available on your OpenAI key
- **Fix: LinkedIn posts were not being published** — the wizard was sending `post` as the field name but the backend expected `post_text`; every publish call sent empty text to LinkedIn; also fixed `schedule_at` → `scheduled_at` for scheduled posts
- **Fix: hashtags now included in LinkedIn post** — hashtags generated by AI are now appended to the post text before sending to LinkedIn
- **Fix: image generation silent failure** — newer OpenAI models (gpt-image-1, gpt-5.5) return `b64_json` data not a URL; the app now handles both and serves the image as a data URL; removed `response_format` parameter which was rejected as unknown by newer models
