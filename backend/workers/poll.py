"""Email polling subsystem — provider cache, folder checks, and the poll cycle.

Extracted from main.py to keep that file under the 500-line limit. All poll
state (last-poll timestamps, provider/folder caches, the poll lock) lives here.
main.py reads the last-poll status via poll_status() rather than importing the
module-level globals directly (importing an int/str binds the value at import
time, so a getter is required to see live updates).
"""
import os
import asyncio
...
def poll_status() -> dict:
    """Live snapshot of last-poll state (getter so importers see updates)."""
    return {
