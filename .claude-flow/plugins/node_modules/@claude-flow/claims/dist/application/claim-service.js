...
import { ClaimOperationError, durationToMs, } from '../domain/types.js';
import { createClaimCreatedEvent, createClaimReleasedEvent, createClaimExpiredEvent, createClaimStatusChangedEvent, createHandoffRequestedEvent, createHandoffAcceptedEvent, createHandoffRejectedEvent, createReviewRequestedEvent, } from '../domain/events.js';
// =============================================================================
...
export class ClaimService {
    ...
    async release(issueId, claimant) {
        ...
        if (claim.claimant.id !== claimant.id) {
            throw new ClaimOperationError('UNAUTHORIZED', `Claimant ${claimant.name} does not own the claim on issue ${issueId}`);
        }
        // Check for pending handoffs
        const pendingHandoff = claim.handoffChain?.find((h) => h.status === 'pending');
        if (pendingHandoff) {
            throw new ClaimOperationError('HANDOFF_PENDING', `Cannot release claim with pending handoff to ${pendingHandoff.to.name}`);
        }
        // Update claim status
        ...
        if (previousStatus !== 'released') {
            const statusEvent = createClaimStatusChangedEvent(claim.id, issueId, previousStatus, 'released');
    ...
    async requestHandoff(issueId, from, to, reason) {
        ...
        if (claim.claimant.id !== from.id) {
            throw new ClaimOperationError('UNAUTHORIZED', `Claimant ${from.name} does not own the claim on issue ${issueId}`);
        }
        // Check for existing pending handoffs
        const existingPending = claim.handoffChain?.find((h) => h.status === 'pending');
        if (existingPending) {
            throw new ClaimOperationError('HANDOFF_PENDING', `A handoff to ${existingPending.to.name} is already pending`);
        }
        ...
        const statusEvent = createClaimStatusChangedEvent(claim.id, issueId, 'active', 'pending_handoff');
        await this.eventStore.append(statusEvent);
    }
    async acceptHandoff(issueId, claimant) {
        ...
        if (!pendingHandoff) {
            throw new ClaimOperationError('HANDOFF_NOT_FOUND', `No pending handoff found for claimant ${claimant.name}`);
        }
        // Check claimant's workload
        const currentClaimCount = await this.claimRepository.countByClaimant(claimant.id);
        const maxClaims = claimant.maxConcurrentClaims ?? 5;
        if (currentClaimCount >= maxClaims) {
            throw new ClaimOperationError('MAX_CLAIMS_EXCEEDED', `Cannot accept handoff: claimant ${claimant.name} at max capacity`);
        }
        ...
        await this.claimRepository.save(claim);
        // Emit events
        const acceptEvent = createHandoffAcceptedEvent(claim.id, issueId, pendingHandoff.id, previousClaimant, claimant);
        await this.eventStore.append(acceptEvent);
        const statusEvent = createClaimStatusChangedEvent(claim.id, issueId, 'pending_handoff', 'active');
        await this.eventStore.append(statusEvent);
    }
    async rejectHandoff(issueId, claimant, reason) {
        ...
        await this.claimRepository.save(claim);
        // Emit events
        const rejectEvent = createHandoffRejectedEvent(claim.id, issueId, pendingHandoff.id, pendingHandoff.from, claimant, reason);
    ...
    async updateStatus(issueId, status, note) {
        ...
        if (!validTransitions.includes(status)) {
            throw new ClaimOperationError('INVALID_STATUS_TRANSITION', `Cannot transition from ${claim.status} to ${status}`, { currentStatus: claim.status, requestedStatus: status, validTransitions });
        }
        const previousStatus = claim.status;
        claim.status = status;
        claim.lastActivityAt = new Date();
        ...
        await this.claimRepository.save(claim);
        // Emit event
        const statusEvent = createClaimStatusChangedEvent(claim.id, issueId, previousStatus, status, note);
        await this.eventStore.append(statusEvent);
    }
    async requestReview(issueId, reviewers) {
        ...
        for (const reviewer of reviewers) {
            ...
            if (!exists) {
                throw new ClaimOperationError('CLAIMANT_NOT_FOUND', `Reviewer ${reviewer.name} not found`);
            }
        }
        // Update claim
        const previousStatus = claim.status;
        ...
        await this.claimRepository.save(claim);
        // Emit events
        const reviewEvent = createReviewRequestedEvent(claim.id, issueId, reviewers, claim.claimant);
        await this.eventStore.append(reviewEvent);
        if (previousStatus !== 'in_review') {
            const statusEvent = createClaimStatusChangedEvent(claim.id, issueId, previousStatus, 'in_review');
            await this.eventStore.append(statusEvent);
        }
    }
    // ==========================================================================
    ...
    async expireStale(maxAge) {
        ...
        for (const claim of staleClaims) {
            ...
            await this.claimRepository.save(claim);
            // Emit events
            const expireEvent = createClaimExpiredEvent(claim.id, claim.issueId, claim.claimant, claim.lastActivityAt.getTime());
            await this.eventStore.append(expireEvent);
            const statusEvent = createClaimStatusChangedEvent(claim.id, claim.issueId, previousStatus, 'expired');
            await this.eventStore.append(statusEvent);
