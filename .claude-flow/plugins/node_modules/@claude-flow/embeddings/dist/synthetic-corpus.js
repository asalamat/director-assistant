 ...
 */
export function buildSyntheticTopicCorpus(options) {
    ...
    if (topics <= 0)
        throw new Error('topics must be > 0');
    if (docsPerTopic <= 0)
        throw new Error('docsPerTopic must be > 0');
    if (dim < topics)
        throw new Error(`dim (${dim}) must be >= topics (${topics})`);
    const rng = mulberry32(seed);
    const dupCount = Math.round(docsPerTopic * duplicateRatio);
    const divCount = docsPerTopic - dupCount;
    const entries = [];
    const relevantByTopic = Array.from({ length: topics }, () => new Set());
    const topicQueryVectors = [];
