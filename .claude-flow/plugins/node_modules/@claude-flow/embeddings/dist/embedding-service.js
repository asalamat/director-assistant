 ...
 * - Cache hit: <1ms
 */
// ADR-121 Phase 1 — BaseEmbeddingService + LRUCache moved to a
// dedicated file so sibling provider files can extend the base
// without circular-import TDZ. Re-exported below for back-compat.
import { BaseEmbeddingService, LRUCache } from './embedding-service-base.js';
import { RvfEmbeddingService } from './rvf-embedding-service.js';
import { RuvectorOnnxEmbeddingService } from './ruvector-onnx-embedding-service.js';
// LRUCache + BaseEmbeddingService now live in ./embedding-service-base.ts
// (see import block above). Re-exported here so any existing consumer
// that imported them from this file keeps working.
export { LRUCache, BaseEmbeddingService };
// ============================================================================
...
// ============================================================================
/**
 * ADR-121 Phase 4 — module-scoped flag so the deprecation warning
 * fires exactly once per process, regardless of how many
 * `TransformersEmbeddingService` instances get constructed.
 */
let xenovaDeprecationWarned = false;
/**
 * @deprecated since `@claude-flow/embeddings@3.0.0-alpha.22` (ADR-121
 * Phase 4). Use `provider: 'ruvector-onnx'` instead — it's 34× smaller
 * to install (7.13 MB vs 247.20 MB), runs WASM+SIMD ONNX, and is
 * portable to Node/browsers/Workers/Deno. The underlying
 * `@xenova/transformers` package was retired upstream and renamed to
 * `@huggingface/transformers@4.x`; carrying it in the runtime tree of
 * every embedding install is a growing liability.
 *
 * Scheduled for removal in 4.0. See ADR-121 for the migration path.
 */
export class TransformersEmbeddingService extends BaseEmbeddingService {
    ...
    constructor(config) {
        ...
        // ADR-121 Phase 4 — one-shot deprecation warning per process.
        // Suppressed when CLAUDE_FLOW_SUPPRESS_DEPRECATION=1 so CI smokes
        // that intentionally exercise the legacy path don't accidentally
        // catch the warning as a test failure.
        if (!xenovaDeprecationWarned && process.env.CLAUDE_FLOW_SUPPRESS_DEPRECATION !== '1') {
            xenovaDeprecationWarned = true;
            // process.emitWarning gets the warning into the standard Node
            // diagnostic channel where node --trace-deprecation surfaces it,
            // and where test runners can intercept via `process.on('warning')`.
            process.emitWarning("@claude-flow/embeddings: 'transformers' provider is deprecated " +
                "(ADR-121 Phase 4). Use provider: 'ruvector-onnx' — 34× smaller " +
                "install, WASM+SIMD ONNX, portable to Node/browsers/Workers/Deno. " +
                'Scheduled for removal in 4.0.', { type: 'DeprecationWarning', code: 'CLAUDE_FLOW_EMBEDDINGS_XENOVA_DEPRECATED' });
        }
    }
    async initialize() {
        if (this.initialized)
        ...
        try {
            ...
            if (!handle) {
                throw new Error('No transformers package available. Install @huggingface/transformers (preferred) ' +
...
export function createEmbeddingService(config) {
    switch (config.provider) {
        ...
        case 'ruvector-onnx':
            // ADR-121 Phase 1 — WASM+SIMD ONNX provider via
            // ruvector-onnx-embeddings-wasm. The provider class extends
            // BaseEmbeddingService — both lived in the same module file
            // until alpha.19 caused a TDZ; moved to a dedicated base file
            // so the import graph stays acyclic.
            return new RuvectorOnnxEmbeddingService(config);
        default:
            throw new Error(`Unknown embedding provider: '${config.provider}'. ` +
                `Use 'ruvector-onnx' (Phase 1 recommended), 'agentic-flow', 'transformers', 'openai', 'rvf', or 'mock' (tests only).`);
    }
}
/**
...
export async function createEmbeddingServiceAsync(config) {
    ...
    if (provider === 'auto') {
        ...
        throw new Error("[embeddings] No real embedding provider available for 'auto'. " +
