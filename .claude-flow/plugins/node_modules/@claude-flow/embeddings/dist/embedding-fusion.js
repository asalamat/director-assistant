 ...
 */
export function averageEmbeddings(vectors, options = {}) {
    ...
    if (weights.length !== vectors.length) {
        throw new Error(`averageEmbeddings: weights.length (${weights.length}) must match vectors.length (${vectors.length})`);
    }
    ...
    for (let i = 1; i < vectors.length; i++) {
        if (vectors[i].length !== dim) {
            throw new Error(`averageEmbeddings: vector ${i} has dim ${vectors[i].length}, expected ${dim}`);
        }
    }
 ...
 */
export function isUnitNorm(v, tolerance = 1e-5) {
    let sq = 0;
    for (let i = 0; i < v.length; i++)
        sq += v[i] * v[i];
    return Math.abs(Math.sqrt(sq) - 1) < tolerance;
}
//# sourceMappingURL=embedding-fusion.js.map