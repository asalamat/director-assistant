...
import { lazyAdaptiveRoute } from '../lazy-adaptive-router.js';
const DIM = 8;
...
function makeAdapters(textToVec, corpusForTopK) {
    const embed = vi.fn(async (text) => {
        const v = textToVec[text];
        if (!v)
            throw new Error(`unmapped text: ${text}`);
        return v;
    });
    const topK = vi.fn(async (q, k) => {
    ...
    return { embed: { embed }, topK: { topK }, embedSpy: embed, topKSpy: topK };
}
describe('lazyAdaptiveRoute — short-circuit', () => {
    it('MMR signal fires → variant + hypothetical embeds SKIPPED', async () => {
        ...
        const result = await lazyAdaptiveRoute(adapters.embed, adapters.topK, { queryText: 'q', variantTexts: ['v1', 'v2'], hypotheticalTexts: ['h1'] });
    ...
    it('No early signal → embeds variants → fires RRF', async () => {
        ...
        const result = await lazyAdaptiveRoute(adapters.embed, adapters.topK, { queryText: 'q', variantTexts: ['v1', 'v2', 'v3'], hypotheticalTexts: ['h1'] });
    ...
    it('No early signal AND no variant signal → embeds hypotheticals → fires HyDE', async () => {
        ...
        const result = await lazyAdaptiveRoute(adapters.embed, adapters.topK, { queryText: 'q', variantTexts: ['v1', 'v2'], hypotheticalTexts: ['h1', 'h2'] });
...
describe('lazyAdaptiveRoute — cost accounting', () => {
    it('embedsUsed + embedsSaved === embedsEager', async () => {
        ...
        const r = await lazyAdaptiveRoute(adapters.embed, adapters.topK, { queryText: 'q', variantTexts: ['v1'], hypotheticalTexts: ['h1'] });
    ...
    it('zero variants and hypotheticals → just the question', async () => {
        ...
        const r = await lazyAdaptiveRoute(adapters.embed, adapters.topK, { queryText: 'q', variantTexts: [], hypotheticalTexts: [] });
...
describe('lazyAdaptiveRoute — option pass-through', () => {
    it('custom duplicateThreshold changes the decision', async () => {
        ...
        const r1 = await lazyAdaptiveRoute(a1.embed, a1.topK, { queryText: 'q', variantTexts: ['v1'], hypotheticalTexts: ['h1'] });
        ...
        const r2 = await lazyAdaptiveRoute(a2.embed, a2.topK, { queryText: 'q', variantTexts: ['v1'], hypotheticalTexts: ['h1'] }, { duplicateThreshold: 0.95 });
        ...
        const r3 = await lazyAdaptiveRoute(a3.embed, a3.topK, { queryText: 'q', variantTexts: ['v1'], hypotheticalTexts: ['h1'] });
        expect(r3.decision.primitive).toBe('plain');
    });
});
//# sourceMappingURL=lazy-adaptive-router.test.js.map