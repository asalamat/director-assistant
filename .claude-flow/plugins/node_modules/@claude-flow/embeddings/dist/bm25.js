 ...
 */
export function buildBm25Index(corpus, options = {}) {
    const k1 = options.k1 ?? 1.5;
    const b = options.b ?? 0.75;
    if (k1 <= 0)
        throw new Error('k1 must be > 0');
    if (b < 0 || b > 1)
        throw new Error('b must be in [0, 1]');
    const docs = [];
    const df = new Map();
    let totalLen = 0;
 ...
 * (query, doc) pair.
 */
export function bm25Score(queryTokens, doc, index) {
 ...
 * query tokens.
 */
export function bm25TopK(query, index, k) {
 ...
 */
export function idfOf(token, index) {
 ...
 */
export function queryMeanIdf(query, index) {
    const queryTokens = Array.from(new Set(tokenize(query)));
    if (queryTokens.length === 0)
        return 0;
    let total = 0;
