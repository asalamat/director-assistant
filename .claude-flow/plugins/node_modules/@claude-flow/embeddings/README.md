...
[![Footprint](https://img.shields.io/badge/Install-7.13%20MB-brightgreen.svg)](https://github.com/ruvnet/ruflo/issues/2036)
[![Quantization](https://img.shields.io/badge/int8%20recall-1.0000-brightgreen.svg)](https://github.com/ruvnet/ruflo/blob/main/v3/docs/adr/ADR-121-embeddings-ruvector-upgrade.md)
[![RaBitQ](https://img.shields.io/badge/RaBitQ-32%C3%97%20reduction-brightgreen.svg)](https://github.com/ruvnet/ruflo/blob/main/v3/docs/adr/ADR-121-embeddings-ruvector-upgrade.md)
[![DiskANN](https://img.shields.io/badge/DiskANN-0.1%20ms%2Fquery-brightgreen.svg)](https://github.com/ruvnet/ruflo/blob/main/v3/docs/adr/ADR-121-embeddings-ruvector-upgrade.md)
...
## SOTA at a glance (ADR-121)

Measured 2026-05-17 via [`scripts/benchmark-embeddings-sota.mjs`](https://github.com/ruvnet/ruflo/blob/main/scripts/benchmark-embeddings-sota.mjs), gated in CI as `embeddings-sota-benchmark`:

| Capability | Measured | Acceptance gate |
|------------|----------|-----------------|
| **Install footprint** (`provider: 'ruvector-onnx'`) | **7.13 MB** | ≥30% smaller than Xenova → measured **97.1% (34.7× smaller)** |
| **int8 quantization recall** (`SearchableEmbeddingCache`, `quantize: 'int8'`) | **1.0000 mean** | ≥0.98 mean recall on unit-normalized vectors ✅ |
| **RaBitQ memory reduction** (`RabitqSnapshot`) | **32×** (theoretical, 10k×384) | ≥32× ✅ |
| **Hyperbolic Poincaré ops** (`@ruvector/attention`) | **~1 μs/op** (native NAPI) | Backend reachable + finite output ✅ |
| **DiskANN/Vamana** (`DiskannSnapshot`, billion-scale on-SSD) | **200 vec build in 203 ms · 0.1 ms/query** | Search latency <10 ms/query on 200-vec corpus ✅ |
| **ruvector MCP sidecar probe** | **83 tools / 15 groups** (when installed) | Probe returns structured report without throwing ✅ |
| **Xenova provider deprecation** (`provider: 'transformers'`) | One-shot warning per process | `CLAUDE_FLOW_EMBEDDINGS_XENOVA_DEPRECATED` code present ✅ |

See [ADR-121](https://github.com/ruvnet/ruflo/blob/main/v3/docs/adr/ADR-121-embeddings-ruvector-upgrade.md) for the per-phase rationale and [#2036](https://github.com/ruvnet/ruflo/issues/2036) for the iteration log.
